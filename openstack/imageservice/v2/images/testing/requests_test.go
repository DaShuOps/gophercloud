...
import (
	...
	fakeclient "github.com/DashuOps/gophercloud/testhelper/client"
)
...
func TestListImage(t *testing.T) {
	...
	err := pager.EachPage(func(page pagination.Page) (bool, error) {
		pages++
		t.Logf("Page %v", page)
		imageResult, err := images.ExtractImages(page)
		if err != nil {
			return false, err
		}

		for _, i := range imageResult {
...
func TestAllPagesImage(t *testing.T) {
	...
	th.AssertNoErr(t, err)
	imageResult, err := images.ExtractImages(pages)
	th.AssertNoErr(t, err)
	th.AssertEquals(t, 3, len(imageResult))
}
